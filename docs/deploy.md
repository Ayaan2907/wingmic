...
1. In the Railway project, add a **Cron** service (or a scheduled job) that runs at `0 3 * * *` (nightly, UTC) with start command `bun run bay:ingest` from the repo root — the script builds the `@wingmic/bay` workspace dependency first (its `dist/` output is what the job imports; a fresh or stale checkout without the build fails at module resolution), then runs the job. **`TURSO_DB_URL` must be set on that service** — the job refuses to run (exit 2) against a missing or cwd-relative `file:./` url, because that would silently write an ephemeral local file while the bay never updates. The cron service needs the same `TURSO_DB_URL` (+ `TURSO_AUTH_TOKEN` if set) as the web service; no other secrets.

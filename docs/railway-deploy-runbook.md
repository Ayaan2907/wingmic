...
**Email sender:** production ignores `RESEND_FROM` in Railway variables and uses `DEFAULT_RESEND_FROM` from `apps/app/lib/config/resend-from.ts` (`wingmic <info@mail.wingmic.xyz>`). Delete any stale `RESEND_FROM` variable after deploy — the app logs a warning if one is present but ignored.
